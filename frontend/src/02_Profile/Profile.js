    ...
    <div className="modal-overlay" onClick={isLoading ? null : onClose}>
      ...
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-footer">
          ...
          {showCancel && (
            <button className="modal-btn cancel-btn" onClick={isLoading ? null : onClose} disabled={isLoading}>
              {cancelText}
            </button>
          )}
        </div>
...
function Profile() {
  ...
  const [isModalLoading, setIsModalLoading] = useState(false); // สำหรับ spinner ใน modal
  ...
  const handleLogout = () => {
    showModal({
      ...
      onConfirm: async () => {
        ...
        closeModal();
        const shopIdVal = localStorage.getItem("shopId") || "";
        navigate(`/${shopIdVal ? `?shopId=${shopIdVal}` : ''}`);
      }
    });
  };
    ...
    <div className="profile-container">
      ...
      <div className="profile-wrapper">
        <header className="profile-header">
          <button className="back-btn" onClick={handleGoBack} style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            ...
            <div className="avatar-section">
              ...
              <div className="avatar-actions">
                <label className="upload-btn" htmlFor="avatar-upload">
                  {(tempUser.avatar || previewUrl || user.avatar) ? (
                  ...
                  <span>{(tempUser.avatar || previewUrl || user.avatar) ? "แก้ไข" : "อัปโหลด"}</span>
                </label>
